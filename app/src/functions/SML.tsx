...
export function SML() {
  ...
  if (isLoading) return (
    <div className="h-full flex flex-col items-center justify-center gap-4">
      <div className="text-term-amber text-[11px] uppercase tracking-[0.4em] animate-pulse font-bold">
        ● Initializing SML War Room Beast...
      </div>
      <div className="text-term-muted text-[9px] uppercase tracking-widest">
        Synchronizing neural link — fetching live tape
      </div>
    </div>
  );
  ...
  if (error && !data) return (
    <div className="h-full flex flex-col items-center justify-center gap-6 p-8">
      <div className="w-full max-w-md border border-term-red/30 bg-term-red/5 rounded p-6 flex flex-col gap-4">
        <div className="flex items-center gap-3">
          <span className="text-term-red text-xl font-black">⚠</span>
          <div>
            <div className="text-term-red text-[11px] font-black uppercase tracking-[0.3em]">
              Council Syncing...
            </div>
            <div className="text-term-muted text-[9px] mt-0.5 uppercase tracking-widest">
              War Room Beast establishing connection
            </div>
          </div>
        </div>
        <div className="text-[9px] text-term-muted font-mono bg-black/40 p-2 rounded border border-term-border">
          {(error as Error).message}
        </div>
        <button
          onClick={() => qc.invalidateQueries({ queryKey: ["terminal-v2"] })}
          className="w-full py-2 border border-term-amber/50 text-term-amber text-[10px] uppercase tracking-widest font-bold hover:bg-term-amber/10 transition-colors rounded"
        >
          {isFetching ? "● Reconnecting..." : "⟳ Reconnect War Room Beast"}
        </button>
        <div className="text-[8px] text-term-muted text-center">
          Auto-retrying every 5s · Backend: /api/terminal
        </div>
      </div>
    </div>
  );
      ...
      <div className="bg-term-panel2 border-b border-term-amber/30 px-4 py-2 flex items-center justify-between shadow-[0_0_15px_rgba(255,140,0,0.1)]">
        ...
        <div className="flex items-center gap-4 text-right">
          <div className="flex flex-col">
            ...
            <span className={cn("font-bold text-[11px]", isFetching ? "text-term-amber animate-pulse" : "text-term-green animate-pulse")}>
              {isFetching ? "● SYNCING TAPE..." : "● LIVE TAPE SYNCHRONIZED"}
            </span>
          </div>
        </div>
      </div>
