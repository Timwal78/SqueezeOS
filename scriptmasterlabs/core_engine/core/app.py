...
from core.api.apikey_bp import apikey_bp
...
def create_app():
    ...
    if not _IS_SERVERLESS:
        init_services()
        start_whale_stalker()
        # Load API keys from disk + sync from Stripe (non-blocking)
        try:
            import threading as _t
            from core.apikey_store import load_from_disk, sync_from_stripe
            load_from_disk()
            _t.Thread(target=sync_from_stripe, daemon=True, name="stripe-sync").start()
        except Exception as _e:
            logger.warning(f"[APIKeys] Startup load failed: {_e}")
