...
export function createInMemoryReplayStore(): AntiReplayStore {
  const store = new Map<string, number>();
  return {
    has(key) {
      const exp = store.get(key);
      if (exp === undefined) return false;
      if (Date.now() > exp) { store.delete(key); return false; }
      return true;
    },
    set(key, expiresAt) { store.set(key, expiresAt); },
    sweep() {
      const now = Date.now();
      for (const [k, exp] of store) if (now > exp) store.delete(k);
    },
  };
}
...
const RLUSD_ISSUERS: Record<string, string> = {
...
export async function verifyPayment(
  ...
  store: AntiReplayStore = _defaultStore
