...
def _execute_tradier_options(sym: str, action: str, resolution: dict, price: float) -> dict:
    ...
    try:
        ...
        # Fetch nearest ATM option symbol from Tradier
        ...
        # Filter by type (call/put) and pick ATM (nearest to current price)
        ...
        atm = min(filtered, key=lambda o: abs(float(o.get("strike", 0)) - price))
        option_symbol = atm["symbol"]
        ask_price     = float(atm.get("ask") or atm.get("last") or 0)
