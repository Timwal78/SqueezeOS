...
- **Not applied to the shipped defaults by this pass.** Unlike Sovereign Squeeze/Quad-Score (where the validated config was adopted *before* the engine went live), this engine is **already live-armed on real money** — silently changing its defaults would change what an already-running real-money engine trades on the next deploy. Whether to adopt this improved-but-imperfect config is the operator's own call, stated here rather than applied unilaterally.
...
### Audit finding, not yet acted on: the account-wide options cap may now be a real bottleneck across 7 competing systems

Full-codebase audit (2026-08-01) of all 7 live primary systems found `IAM_MAX_OPEN_CALLS`/`IAM_MAX_OPEN_PUTS` (default 1 each, `iam_executor.py`) are **account-wide, not per-system** — verified in `_execute_tradier_options()`: when the cap is hit, the signal is dropped entirely (`{"status": "skipped"}`), with no fallback to equity. This cap was set 2026-07-30 as a deliberate "let me get comfortable" choice when far fewer systems traded options live. With 7 systems now live and potentially all routing through `IAM_INSTRUMENT=options`, whichever system opens a call first blocks every other system's BUY signal until it closes, regardless of that signal's own evidence quality. Similarly, `IAM_MAX_ORDERS_PER_DAY=5` / `IAM_MAX_NOTIONAL_PER_DAY=$2000` (code defaults) are also shared account-wide across all 7 systems — this file references a prior decision to change these but the actual note could not be located, so **the real live Render values for these three are unverified from this sandbox** (no Render dashboard access, same standing limitation as every live-arming decision here). **Not changed by this pass** — these are real-money risk-exposure parameters, and per this file's own standing rule, changing them needs the operator's own explicit numbers, not an inferred value.
