...
- **Scope check, so this isn't overstated:** this only affects the IAM-primary queue (`iam_pending_bp.py`) — the parallel `tv_pending` queue (`core/api/tradingview_webhook_bp.py`, raw TradingView-Pine alerts) was not investigated as part of this fix and may or may not share the same pattern; it wasn't part of what was asked here and hasn't been checked.
