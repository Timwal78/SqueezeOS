...
- **NOT added to `IAM_PRIMARY_SYSTEM`.** A validated backtest is not itself a live-arming decision — same standing rule as every other engine in this file. This trades on paper (`IAM_PAPER_MODE=true` default) out of the box; real paper-trade evidence accumulates automatically via the existing Paper Trade Ledger (`system="SML_QUAD_SCORE"`).
