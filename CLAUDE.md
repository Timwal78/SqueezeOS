...
- `breakout_scanner.py`, `sr_matrix_scanner.py`, `sr_zone_pattern_scanner.py`, `sovereign_squeeze_scanner.py`, `quad_score_scanner.py` — `*_SCAN_TOP_N` default raised **10 → 25** each. With CASCADE's own 40 sharing the same global Tradier queue, worst-case simultaneous queue-drain is `(5×25 + 40) × 1.05s ≈ 173s` — comfortably under the shared 300s `SCAN_INTERVAL` with ~42% margin, even in the pessimistic case where every scanner's pass starts at the exact same instant (in practice they don't).
