...
- **Not wired to `iam_executor` or any scanner** — this build stopped at "port + backtest + honest verdict" per explicit scope. `IAM_PAPER_MODE` is unaffected either way. A live-paper scanner wiring pass (matching DRUCK/CIE/Breakout/SR-Matrix's pattern) is a separate, explicit next step if wanted.
