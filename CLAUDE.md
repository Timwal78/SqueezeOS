...
### REMOVED FROM LIVE TRADING ENTIRELY (operator directive, 2026-08-01: "remove it altogether")

Given directly, right after the touch-only test above came back worse than the already-thin shipped config — this engine never cleared the same evidence bar the other 6 live systems did (12-52 trades vs. 20-30+/symbol for CASCADE/Breakout/S/R-Matrix), and testing the operator's own literal reading of the chart didn't rescue it either.

- **`sr_zone_pattern_scanner.py`'s `SR_ZONE_PATTERN_SCAN_ENABLED` now defaults `false`** (was `true`) — the scanner itself no longer starts by default, so it generates **zero** signals, paper or live, without any Render env var edit. This is a stronger removal than just excluding `SML_SR_ZONE_PATTERN` from `IAM_PRIMARY_SYSTEM`: even in `IAM_EXECUTION_MODE=both`/paper mode it would otherwise keep trading paper and accumulating Paper Trade Ledger history for a system the operator explicitly wants gone, not just kept off the broker.
- **Still recommended, for cleanliness (not load-bearing):** drop `SML_SR_ZONE_PATTERN` from Render's `IAM_PRIMARY_SYSTEM` whenever convenient, so the listed primary systems match what's actually running. Not urgent — with the scanner disabled, that system tag can never fire a signal regardless of whether it's still listed there.
- **`scan_budget.py` updated to match** — its dynamic scan-width allocator now treats `SR_ZONE_PATTERN` as off-by-default too (`_DEFAULT_ENABLED` map), so the other 4 secondary scanners (Breakout/S-R-Matrix/Sovereign-Squeeze/Quad-Score) automatically widen to fill its share `(142-40)//4 = 25` instead of a smaller `//5 = 20` that assumed a scanner no longer running. Before this fix the two modules' defaults would have disagreed — the scanner off, but the allocator still budgeting a share for it that nothing would ever use.
- **Nothing was deleted.** `sr_zone_pattern_engine.py`, `sr_zone_pattern_scanner.py`, `core/api/sr_zone_pattern_bp.py`, and every test/doc stay in the repo, matching this codebase's "measure, don't delete" convention for every engine here — set `SR_ZONE_PATTERN_SCAN_ENABLED=true` to bring it back if ever revisited.
- Tests: `tests/test_scan_budget.py` rewritten around the new 4-scanner default baseline (previously 5) — explicit new tests prove SR_ZONE_PATTERN is absent from the active set by default AND that its absence genuinely widens the other 4's share, and that it can still be explicitly re-enabled to restore the 5-way split. All pass against the real, unmodified allocator.
...
- **⚠️ CONFIRMED LIVE 2026-08-01** — operator screenshot of the real Render `IAM_PRIMARY_SYSTEM` value showed `SML_QUAD_SCORE` already present, superseding the "not yet confirmed" claim originally written here.
- **⚠️ `SML_SR_ZONE_PATTERN` should now be dropped from whatever value is on Render** — see that engine's own "REMOVED FROM LIVE TRADING ENTIRELY" section above (2026-08-01, operator directive: "remove it altogether"). Its scanner is disabled by default now regardless (so it can't fire even if the tag is still listed), but the Render value should be cleaned up to match next time it's edited, rather than perpetuating a stale reference here. The recommended roster going forward is `SML_BREAKOUT,SML_CASCADE,SML_MM_V4,SML_SR_MATRIX,SML_SOVEREIGN_SQUEEZE,SML_QUAD_SCORE` (i.e. the 2026-07-31 baseline with `SML_SR_ZONE_PATTERN` removed and `SML_QUAD_SCORE` added) — **not confirmed as the literal current Render value**, since no sandbox here has ever had Render access; append/remove against whatever is actually set there.
- **Stop-loss is automatic, not something built per-engine** — `IAM_STOP_LOSS_PCT` (default 3.0%) places a real GTC stop-sell order on every live BUY fill regardless of which system triggered it, same as every other primary system.
