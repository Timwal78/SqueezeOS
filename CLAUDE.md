...
- **This does NOT flip any live-trading switch, but the evidence bar it clears is now comparable to CASCADE/Breakout/S/R-Matrix's pre-arming backtests.** `sovereign_squeeze_scanner.py` still feeds `iam_executor` under the default `IAM_PAPER_MODE=true` — it trades on paper out of the box. Nobody has added `SML_SOVEREIGN_SQUEEZE` to `IAM_PRIMARY_SYSTEM`; given this evidence that would now be a defensible operator decision, not one made here.
