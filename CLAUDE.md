...
- **Known open question, not yet resolved:** `_pdt_allowed()`'s `PDT_BALANCE_LIMIT` is configured at $2,100 — the real FINRA PDT equity requirement for margin accounts is $25,000. Operator confirmed (2026-07-29) the account is margin, under $25k equity — meaning the real PDT rule (3 day-trades per rolling 5 business days below $25k) does apply, and this constant is ~12x too low to reflect that correctly. Not yet fixed — flagged here so it isn't lost; fixing the threshold, not removing the gate.
...
- **Now wired to live PAPER execution (2026-07-25, operator said "MM live" — confirmed via clarifying question this meant paper wiring, not real orders):** `mm_intel_scanner.py` — background loop (started in `core/app.py` beside `imo_scanner`/`orb_scanner`/`druck_scanner`/`cie_scanner`/`breakout_scanner`/`sr_matrix_scanner`/`gamma_pin_scanner`), pulls real intraday bars via DataManager (`MM_INTEL_TIMEFRAME=5MIN` default, matching the backtest's own granularity — needs Polygon/Alpaca for intraday, idles honestly and logs why on a Tradier-only deployment, same as ORB/DRUCK). Routes to `iam_executor.execute_async()` tagged `system="SML_MM_INTEL"`.
  ...
  - **This does NOT flip any live-trading switch.** `mm_intel_scanner.py` feeds `iam_executor` under the exact same `IAM_PAPER_MODE=true` default as every other engine. Nobody has added `SML_MM_INTEL` to `IAM_PRIMARY_SYSTEM` — that stays a separate, explicit, future decision, especially given the no-options-modeled caveat above (this backtest never tested actual 0DTE premium/theta economics).
...
## SML CVD Regime Desk — 7 bugs fixed in an operator-submitted script, BACKTEST DONE, verdict NO DEMONSTRATED EDGE (2026-07-30)
...
- **BACKTEST VERDICT: NO DEMONSTRATED EDGE — do not arm this live.** Real 5-min bars (SPY/QQQ/IWM/NVDA/TSLA, 2026-07-20 to 2026-07-29, 8 sessions, 624 bars/symbol, Robinhood MCP — same real-data channel as DRUCK/CIE/Breakout/MM-Intel). **102 trades, 41.2% win rate, profit factor 0.989, −0.30% summed; only NVDA positive (PF 1.905), 4 of 5 symbols lost.** Four sensitivity configs all land in a PF 0.875–1.071 band — nothing wins decisively, which is the signature of no measurable edge rather than one mis-set parameter. The best row (confirmed-only, PF 1.071) is inside noise for this sample and was deliberately **not** promoted to the default. Shorts netted positive and longs negative, but every symbol fell over the window, so that is market direction, not a bearish edge.
- **This verdict is bounded, and deliberately not stated more strongly than the evidence:** 8 sessions in a single down-trending regime is thin — the same evidentiary class as CIE's "inconclusive" and Trade Desk v3's "thin evidence," NOT the 29-56-session windows behind the clean ORB/DRUCK negative verdicts. No options economics modelled (directional %-move only, same disclosed convention as `breakout_engine.py`/`mm_intel_engine.py`) — but that inference direction is safe: if the underlying read doesn't pay before theta is charged, theta can't rescue it. No commission/slippage. "CVD" is a bar-range proxy from OHLCV, not true bid/ask delta (same proxy class as CIE's OFI/DLMD labelling).
