...
- **This still does not flip anything live by itself.** Going actually-live still requires the operator to add `SML_SQUEEZE_FUEL` to `IAM_PRIMARY_SYSTEM` on Render directly — no sandbox here has ever had that access. Full current recommended value: `IAM_PRIMARY_SYSTEM=SML_CASCADE,SML_SR_MATRIX,SML_BREAKOUT,SML_MM_V4,SML_SR_ZONE_PATTERN,SML_SQUEEZE_FUEL`.
