...
- **No sandbox here has ever had Render dashboard access — the operator still has to make this exact edit directly.** `sovereign_squeeze_scanner.py` feeds `iam_executor` under the same `IAM_PAPER_MODE`/`IAM_AUTO_TRADING`/`IAM_EXECUTION_MODE` flags every other primary system already shares — those are already set live for CASCADE/Breakout/S/R-Matrix/S/R-Zone-Pattern (see their sections above). The ONE remaining lever specific to this engine is `IAM_PRIMARY_SYSTEM`: add `SML_SOVEREIGN_SQUEEZE` to whatever comma-separated value is currently set on the `squeezeos-api` Render service, e.g. `IAM_PRIMARY_SYSTEM=SML_CASCADE,SML_BREAKOUT,SML_SR_MATRIX,SML_MM_V4,SML_SR_ZONE_PATTERN,SML_SOVEREIGN_SQUEEZE` (append to whatever the real current value is — don't blindly overwrite it if it has drifted from this baseline).
...
- **Until that Render env var is actually changed, this still trades on PAPER only** — building/documenting the decision is not the same event as the signal actually reaching the broker. `IAM_PAPER_MODE=true` remains the default for anyone who hasn't explicitly gone live.
